...
public class OddsSyncService : IOddsSyncService
{
    ...
    public async Task SyncEventsAndOddsAsync(string sportKey)
    {
        ...
        if (sport == null)
        {
            Console.WriteLine($"Sport {sportKey} not found in DB, skipping sync");
            return;
        }
        ...
        foreach (var ev in events)
        {
            ...
            if (existingEventInDb == null)
            {
                ...
                existingEventInDb = await _eventRepository.GetByApiIdAsync(ev.ApiId);
                if (existingEventInDb == null)
                {
                    
                    Console.WriteLine($"Failed to persist event {ev.ApiId}");
                    continue;
                }
            }
            else
            ...
            foreach (var odd in ev.Odds)
            {
                ...
                if (teamInDb == null)
                {
                    ...
                    teamInDb = await _teamRepository.GetByNameAsync(odd.Team.TeamName);
                    if (teamInDb == null)
                    {
                        Console.WriteLine($"Failed to persist team {odd.Team.TeamName}");
                        continue;
                    }
                }
